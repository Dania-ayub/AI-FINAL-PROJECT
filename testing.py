...
def makePredictions(inputData, sand_model, silt_model, clay_model, texture_model):
    # Separate features for texture prediction
    texture_features = inputData[['EC', 'ph', 'OM', 'P', 'K']]

    # Separate features for sand, silt, clay predictions
    bands_features = inputData[['band_2', 'band_3', 'band_4', 'band5', 'band_6', 'band_7', 'band8A', 'band_11', 'band_12']]

    # Make predictions for sand, silt, clay
    sand_pred = sand_model.predict(texture_features)
    silt_pred = silt_model.predict(texture_features)
    clay_pred = clay_model.predict(texture_features)

    # Make prediction for texture
    texture_pred = texture_model.predict(bands_features)
